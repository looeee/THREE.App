var World = (function (exports, three_module_js, OrbitControls_js, three_module_js$1, EffectComposer_js, RenderPass_js, ShaderPass_js, GammaCorrectionShader_js, SMAAPass_js, GLTFLoader_js) {
  'use strict';

  const clock = new three_module_js.Clock();
  ...
  class OrbitingPerspectiveCamera extends three_module_js.PerspectiveCamera {
    ...
    setupControls(container) {
      this.controls = new OrbitControls_js.OrbitControls(this, container);
  ...
  class Renderer {
    constructor(container) {
      const renderer = new three_module_js.WebGLRenderer({
  ...
  class PostRenderer {
    constructor(renderer, scene, camera) {
      ...
      const size = this.renderer.getDrawingBufferSize(new three_module_js$1.Vector2());
    ...
    createComposer(size) {
      ...
      if (this.renderer.capabilities.isWebGL2) {
        const parameters = {
          format: three_module_js$1.RGBFormat,
          stencilBuffer: false,
        };

        renderTarget = new three_module_js$1.WebGLMultisampleRenderTarget(
          size.width,
          size.height,
          parameters,
        );
    ...
    createBasePass() {
      const basePass = new RenderPass_js.RenderPass(this.scene, this.camera);
      basePass.name = 'Initial Render Pass';
      this.composer.addPass(basePass);
    }

    createColorPass() {
      this.colorPass = new ShaderPass_js.ShaderPass(GammaCorrectionShader_js.GammaCorrectionShader);
    ...
    createAAPass(size) {
      ...
      this.aaPass = new SMAAPass_js.SMAAPass(size.x, size.y);
  ...
  class Engine {
    constructor(container, debug = false) {
      ...
      this.scene = new three_module_js.Scene();
...
}({}, three_module_js, OrbitControls_js, three_module_js$1, EffectComposer_js, RenderPass_js, ShaderPass_js, GammaCorrectionShader_js, SMAAPass_js, GLTFLoader_js));
